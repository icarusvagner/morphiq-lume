//! Remote implementation of [`serde::Deserialize`] and [`serde::Serialize`] for
//! [`iced::Color`].
//!
//! This implementation deserializes hexadecimal RGB(A) as string to float
//! RGB(A) and back. NOTE: The alpha channel is optional and defaults to #ff or
//! 1.0. `#ffffffff` deserializes to `1.0`, `1.0`, `1.0`, `1.0`.
...
pub(super) fn deserialize_color<'de, D>(
	deserializer: D,
	...
	D: Deserializer<'de>,
{
	...
	if hex_len == HEX_STR_BASE_LEN || hex_len == HEX_STR_ALPHA_LEN {
		let color = hex
			...
			// If the hex is not ASCII or invalid hex, then the iterator will
			// short circuit and fail on `from_str_radix` TODO: This can be cleaned up when `iter_array_chunks` is stabilized (https://github.com/rust-lang/rust/issues/100450)
			.bytes()
		...
		// Alpha isn't always part of the color scheme. The resulting Vec should
		// always have at least three elements. Accessing the first three
		// elements without [slice::get] is okay because I checked the length of
		// the hex string earlier.
		Ok(Color {
			r: color[0],
			g: color[1],
			b: color[2],
...
#[inline]
pub(super) fn serialize_color<S>(
	color: &Color,
	serializer: S,
) -> Result<S::Ok, S::Error>
where
...
mod tests {
	...
	const CATPPUCCIN_PINK: Color =
		Color { r: 245.0 / 255.0, g: 194.0 / 255.0, b: 231.0 / 255.0, a: 1.0 };
	...
	const CATPPUCCIN_PINK_DELEGATE: DelegateTest =
		DelegateTest { color: CATPPUCCIN_PINK };

	const CATPPUCCIN_PINK_ALPHA_DELEGATE: DelegateTest =
		DelegateTest { color: CATPPUCCIN_PINK_ALPHA };
	...
	// Test if deserializing and serializing a color with an alpha channel
	// works.
	#[test]
