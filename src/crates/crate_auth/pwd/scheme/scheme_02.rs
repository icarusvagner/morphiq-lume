...
impl Scheme for Scheme02 {
	...
	fn validate(&self, to_hash: &ContentToHash, pwd_ref: &str) -> Result<()> {
		...
		let parsed_hash_ref =
			PasswordHash::new(pwd_ref).map_err(|_| Error::Hash)?;
...
fn get_argon2() -> &'static Argon2<'static> {
	...
	INSTANCE.get_or_init(|| {
		...
		.expect("Failed to get argon2") // TODO - needs to fail early
	})
}
	...
	fn test_scheme_02_hash_into_b64u_ok() -> Result<()> {
		...
		let fx_res =
			"$argon2id$v=19$m=19456,t=2,\
			 p=1$8F6JYdatQIaeeKbeBl5UUw$TaRnmmbDdQ1aTzk2qQ2yQzPQoZfnKqhrfuTH/\
			 TRP5V4";
