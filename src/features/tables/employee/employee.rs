...
use crate::{
	core::utils::{icon::Icon, message::Message}, features::tables::employee::table::{EmployeeRow, EmployeeTable}, styles::{button::ButtonType, style_type::StyleType}
};
	...
	fn menu_bar(&self) -> MenuBar<'_, Message, StyleType, Renderer> {
		...
		let menu_bar: MenuBar<'_, Message, StyleType, Renderer> = menu_bar!((
			...
			.on_press(Message::Home(HomeMessage::Employee(
				EmployeeMsg::FilteredBy(FilterEmployee::Department)
			)))
