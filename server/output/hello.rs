...
pub mod hello_server {
    ...
    #[async_trait]
    pub trait Hello: std::marker::Send + std::marker::Sync + 'static {
        async fn greet(
            &self,
            request: tonic::Request<super::SayRequest>,
        ) -> std::result::Result<tonic::Response<super::SayResponse>, tonic::Status>;
    }
    #[derive(Debug)]
        ...
        fn call(&mut self, req: http::Request<B>) -> Self::Future {
            match req.uri().path() {
                "/hello.Hello/Greet" => {
                    ...
                    struct GreetSvc<T: Hello>(pub Arc<T>);
                    impl<T: Hello> tonic::server::UnaryService<super::SayRequest>
                    for GreetSvc<T> {
                        ...
                        fn call(
                            ...
                            let fut = async move {
                                <T as Hello>::greet(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
    ...
    impl<T> Clone for HelloServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    impl<T> tonic::server::NamedService for HelloServer<T> {
